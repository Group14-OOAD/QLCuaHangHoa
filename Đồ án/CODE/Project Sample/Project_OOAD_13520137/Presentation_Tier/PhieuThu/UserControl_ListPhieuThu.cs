...
namespace Presentation_Tier
{
    public partial class UserControl_ListPhieuThu : UserControl
    {
        ...
        private string _findMaPT, _findNgayLap;
        ...
        private void gridControl_DSPhieuThu_Load(object sender, EventArgs e)
        {
            ...
            {
                DateTime dateTime = DateTime.Parse(dr["NgayLap"].ToString());
                string dateItem = dateTime.ToString("yyyy-MM-dd");
                if (!comboBox_ngayLap.Properties.Items.Contains(dateItem))
                    comboBox_ngayLap.Properties.Items.Add(dateItem);
            }
             
            comboBox_ngayLap.Properties.Items.Add("--Tất cả--");
